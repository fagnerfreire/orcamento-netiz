...
const dbPath = path.join(dataDir, 'orcamento.db');
...
function initDatabase() {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      const senhaHashAdmin = bcrypt.hashSync('admin123', 10);
      const senhaHashFagner = bcrypt.hashSync('netiz2024', 10);
      ...
      db.get('SELECT COUNT(*) as count FROM usuarios', [], (err, row) => {
        ...
        if (row.count === 0) {
          ...
          db.run(`INSERT INTO usuarios (nome, email, senha, perfil) 
                  VALUES (?, ?, ?, ?)`, 
                  ['Fagner Tavares Freire', 'fagner@netiz.com.br', senhaHashFagner, 'admin'],
                  (err) => {
                    if (err) {
                      console.error('❌ Erro ao criar usuário Fagner:', err);
                    } else {
                      console.log('✅ Usuário criado: fagner@netiz.com.br / netiz2024');
                    }
                  });

          db.run(`INSERT INTO usuarios (nome, email, senha, perfil) 
                  VALUES (?, ?, ?, ?)`, 
                  ['Administrador', 'admin@netiz.com.br', senhaHashAdmin, 'admin'],
                  (err) => {
                    if (err) {
                      console.error('❌ Erro ao criar usuário admin:', err);
                    } else {
                      console.log('✅ Usuário criado: admin@netiz.com.br / admin123');
                    }
                  });
        }
