...
router.post('/', auth, checkPerfil('admin'), (req, res) => {
  const { nome, descricao, tipo, gestores } = req.body;
  ...
  db.run(
    'INSERT INTO departamentos (nome, descricao, tipo) VALUES (?, ?, ?)',
    [nome, descricao, tipo],
    function(err) {
      if (err) {
...
router.put('/:id', auth, checkPerfil('admin'), (req, res) => {
  const { nome, descricao, tipo, ativo, gestores } = req.body;
  ...
  db.run(
    `UPDATE departamentos SET nome = ?, descricao = ?, tipo = ?, ativo = ?, updated_at = CURRENT_TIMESTAMP
     WHERE id = ?`,
    [nome, descricao, tipo, ativo !== undefined ? ativo : 1, req.params.id],
    function(err) {
      if (err) {
